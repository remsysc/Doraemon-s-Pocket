...
- **Can:** look up stock, follow a FEFO-ordered pick list, log picks/receipts in real time (append `RECEIPT`/`RESERVE`/`PICK`/`SALE`/`ADJUSTMENT`/`WRITE_OFF` rows to `INVENTORY_TRANSACTION`), flag count discrepancies at the moment they're found, create/update `Lot` records as part of logging a receipt (received_date, expiry_date, bin_location). Lot writes follow the physical-receipt interpretation: Admin and Warehouse Staff.
...
- ✅ Resolved (2026-08-09): Purchasing Manager has read-only access to `INVENTORY_TRANSACTION`. Only Warehouse Staff and Admin can append transaction rows, because transaction writes represent physical stock movements. This applies to every transaction type (`RECEIPT`/`RESERVE`/`PICK`/`SALE`/`ADJUSTMENT`/`WRITE_OFF`); Purchasing Manager can still read the ledger for purchasing analysis.
...
- ✅ Resolved (2026-08-09): Product deletion is restricted while related Lots exist. The Lot foreign key uses Laravel's default restrictive behavior because preserving Lot and inventory traceability is safer than cascading deletion.
- ✅ Resolved (2026-08-09): `received_date` is a required `dateTime`. The time component records the precise physical receipt event and supports same-day receipt ordering and reconciliation. `expiry_date` remains a nullable calendar `date`.
...
- ✅ Resolved (2026-09-11, Sprint 4): EOQ cost inputs (`order_cost`, `holding_cost_per_unit`) are modeled as non-price operational cost fields on `REORDER_CONFIG`, not on Product.
...
- **Risk:** Role-guarded catalog and Lot writes could drift from the RBAC matrix. — _Mitigation:_ `routes/api.php`, `RoleMiddleware`, resource policies, and regression tests now enforce Category/Product writes for Admin only and Lot writes for Admin + Warehouse Staff; review this matrix at each sprint sign-off.
