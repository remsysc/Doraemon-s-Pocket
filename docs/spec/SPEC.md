...
`inventory_snapshots` contains one derived row per Product SKU. Each ledger row remains tied to one Lot, while its snapshot side effect aggregates at the Product SKU level. Negative `RESERVE` creates a reservation; positive `RESERVE` releases one. `PICK` consumes reserved stock. Clients SHALL NOT directly create, update, or delete snapshot rows.
...
### Reconciliation ### Reconciliation & Audit (Sprint 5 — not yet started, requirement-level only) Audit (Sprint 5 — Implemented)
